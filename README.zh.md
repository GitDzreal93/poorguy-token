...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/benchmark-真数据·小样本-111111?style=flat-square" alt="Real benchmark, small n">
...
<p align="center">
  <sub><strong>实测：</strong>探索任务上下文 &minus;71%、成本 &minus;67%；生成任务输出 &minus;17%。glm-5.2，小样本。<a href="docs/benchmark-2026-07-17.md">benchmark</a></sub>
</p>
...
我们跑了——同样任务、同样模型、注入 skill vs 纯 baseline，token 计数直接来自模型自己的计数器。Claude Code 里的 glm-5.2（不是 Claude，不是 Codex——见报告）。

| 轴 | 任务 | 结果 |
|---|---|---|
| **读得少** | 16 文件仓库内的探索（n=1） | 上下文 **−71%**、成本 **−67%** |
| **写得少** | 单次生成（n=3） | 输出 **−17%**（区间 −11% ~ −23%） |

baseline 过度探索——其中一个任务为 16 个文件的问题烧了 **76k token / $1.02**。skill 组路由到行范围、返回 evidence packet、写得极简。

说人话：读是 n=1（agentic 有噪声——比例当方向性看），仓库小，skill 是强制注入所以真实节省 ≤ 此值，模型是 glm-5.2。写轴省 output 但规则占 input，所以单次成本基本持平；真实缓存会话里省下的 output 占主导。唯一比这更权威的数字，是你自己在账单页做的 A/B。

复现：`python3 scripts/run_benchmark.py write|read`。完整方法 + 原始数据：[docs/benchmark-2026-07-17.md](docs/benchmark-2026-07-17.md)。
...
**benchmark 在哪？**
在这里：[docs/benchmark-2026-07-17.md](docs/benchmark-2026-07-17.md)——所有局限都写明（小样本、标了模型、强制注入）。用 `scripts/run_benchmark.py` 复现。也建议你在自己账单页跑一次 A/B；那个数字仍然比 README 权威。
